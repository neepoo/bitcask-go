...
// ActiveData 当前活跃的磁盘文件的表示，除了他都是older data file
type ActiveData interface {
	// Write 将key和value写入磁盘，返回dirkey需要的结构
	Write(key, value []byte) (mv *key_dir.MemValue, err error)
	// Read 从磁盘读取key对应的value
	Read(mv *key_dir.MemValue) (value []byte, err error)
	// ID 返回文件ID，对应keydir中的file_id
	ID() uint64 // 文件ID，对应keydir中的file_id
	// Close 关闭文件
	Close() error
	// Delete 删除文件
	Delete() error
